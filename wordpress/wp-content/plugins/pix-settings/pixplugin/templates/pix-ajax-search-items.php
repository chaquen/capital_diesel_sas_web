...
if ( !empty($pix_result) ) {
    ...
    foreach ($pix_result as $item) :
        ...
        if ( ! empty($cats) ) {
	        ...
	        $cat_titles_str = isset($cat_titles[0]) ? '<span>'.$cat_titles[0].'</span>' : '';
        }
        ...
        <li class="pix__searchAjax_product">
            ...
            <div class="pix__searchAjax_product-info">
                '.$cat_titles_str.'
                <h6><a href="'.get_the_permalink($item->ID).'">'.get_the_title($item->ID).'</a></h6>
                <div>
                    <div class="pix__searchAjax_product-price">
                        '.$_product->get_price_html().'
                    </div>
                    '.apply_filters( 'woocommerce_loop_add_to_cart_link',
                        sprintf( '<a href="%s" data-quantity="%s" class="%s btn btn-sm btn-danger" title="%s" %s><i class="pix-flaticon-shopping-bag-3"></i></a>',
